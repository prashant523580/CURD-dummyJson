...
export default function Product({products} : any) {
  ...
  React.useEffect(() => {
    ...
    // console.log(products ,query.productId)
    React.useEffect(() => {
      // console.log(currentProduct)
      console.log(typeof allProducts)
      allProducts && allProducts.filter((product : any) => {
              if(product.id === parseInt(query.productId)){
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      <div className="lg:w-1/2 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
        ...
        <h1 className="text-gray-900 text-3xl title-font font-medium mb-4"> {currentProduct.title}</h1>
        ...
        <p className="leading-relaxed mb-4">{currentProduct.description}</p>
      ...
      <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" src={currentProduct.thumbnail}/>
    </div>
  </div>
</section>
    </div> : 

      <div className='w-15 h-15 border-2 border-skyblue-500 animate-spin'></div>
